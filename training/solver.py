...
class Solver(object):
    ...
    def get_dataset(self):
        ...
        if self.dataset == 'MER31K':
            self.file_dict = np.load('./../split/MER31K/validation_mer31k.npy', allow_pickle=True).item()
    ...
    def validation(self, best_metric, epoch):
        ...
        # loss = self.get_validation_score(epoch)
        score = 1 - loss
    ...
    def get_validation_score(self, epoch):
        ...
        print("roc_auc, pr_auc, loss-->", roc_auc, pr_auc, loss)

        import matplotlib.pyplot as plt
        from sklearn.metrics import roc_curve, auc, precision_recall_curve, average_precision_score

        # ROC curve
        fpr, tpr, _ = roc_curve(gt_array.ravel(), est_array.ravel())
        roc_auc = auc(fpr, tpr)

        # PR curve
        precision, recall, _ = precision_recall_curve(gt_array.ravel(), est_array.ravel())
        pr_auc = average_precision_score(gt_array, est_array)

        # Determine the best AUC
        if roc_auc > pr_auc:
            # Plot ROC curve
            ...
            plt.title('Best Curve: ROC')
        else:
            # Plot PR curve
            plt.plot(recall, precision, color='blue', lw=2, label=f'PR curve (area = {pr_auc:.2f})')
            plt.xlabel('Recall')
            plt.ylabel('Precision')
            plt.title('Best Curve: PR')
