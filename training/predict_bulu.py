...
class Predict(object):
    ...
    def get_dataset(self):
        ...
        if self.dataset == 'MER31K':
            self.file_dict = np.load('./../split/MER31K/test.npy', allow_pickle=True).item()
            self.test_list = list(self.file_dict.keys())
    ...
    def get_tensor(self, fn):
        # load audio
        if self.dataset == 'mtat':
            npy_path = os.path.join(self.data_path, fn.split('/')[1][:-3]) + 'npy'
    ...
    def get_tensor_predict(self, fn):
        # load audio
        if self.dataset == 'mtat':
            npy_path = os.path.join(self.data_path, fn.split('/')[1][:-3]) + 'npy'
        elif self.dataset == 'msd':
        ...
        print("length, input_length", length, self.input_length)
    ...
    def get_test_score(self):
        ...
        for line in tqdm.tqdm(self.test_list):
            ...
            # load and split
            x = self.get_tensor_predict(fn)

            out_predict = self.model(x)
            print("out1", out_predict[0])
            ###
            self.tags = np.load("./../split/mtat/tags.npy")
            # print("self.tags", self.tags)
            # print(" np.sum(self.mlb.transform(self.file_dict[fn]['tags']), axis=0),",  np.sum(self.mlb.transform(self.file_dict[fn]['tags']), axis=0))
            # print("out_predict[0].detach().cpu().numpy().tolist()", out_predict[0].detach().cpu().numpy().tolist())
            result = dict(zip(np.sum(self.mlb.transform(self.file_dict[fn]['tags']), axis=0), out_predict[0].detach().cpu().numpy().tolist()))
            print("result", result)
        ...
        predicted_labels = [MER31K_TAGS[i] for i in np.argmax(est_array, axis=1)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='MER31K', choices=['mtat', 'msd', 'jamendo'])
